...
import {
  ...
  Flame, ArrowUpToLine, ArrowDownToLine, Star, Shield, TrendingUp, X
} from 'lucide-react';
...
export default function AdminPage() {
  ...
  const syncLeaderboard = async (isManual = true) => {
    if (isManual && !window.confirm('Vuoi ricalcolare i punti e gli spareggi per tutti?')) return;
    setSyncing(true);
  ...
  const updateScore = async (id: number) => {
    const h = (document.getElementById(`h-${id}`) as HTMLInputElement).value, a = (document.getElementById(`a-${id}`) as HTMLInputElement).value;
    const isFin = h !== '' && a !== '';
  ...
  const saveBonuses = async (e: React.FormEvent) => {
    ...
    if (!error) { toast.success('Bonus Ufficiali aggiornati!'); await syncLeaderboard(false); }
  };
  ...
  const saveQualif = async () => {
    ...
    if (t && s) { const { error } = await supabase.from('official_bracket').insert([{ stage: s, team_name: t }]); if (!error) { toast.success('Qualificata registrata!'); await syncLeaderboard(false); } }
  };
  ...
  const deleteUser = async (uId: string, name: string) => { if (window.confirm(`Eliminare definitivamente ${name}?`)) { await supabase.from('predictions').delete().eq('user_id', uId); await supabase.from('brackets').delete().eq('user_id', uId); await supabase.from('user_bonus_answers').delete().eq('user_id', uId); await supabase.from('profiles').delete().eq('id', uId); fetchData(); await syncLeaderboard(false); } };
  ...
  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-yellow-500 font-black animate-pulse uppercase">Inizializzazione...</div>;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-4 pb-32 font-sans">
      <header className="text-center mb-12 pt-6">
        ...
        <button onClick={() => syncLeaderboard(true)} disabled={syncing} className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 px-8 py-4 rounded-2xl font-black uppercase text-xs tracking-widest transition-all active:scale-95 shadow-lg shadow-emerald-600/20">
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <section className="bg-slate-900/50 border border-slate-800 rounded-[2rem] overflow-hidden shadow-xl">
          <button onClick={() => setOpenSection({ ...openSection, iscrizioni: !openSection.iscrizioni })} className="w-full p-6 flex items-center justify-between hover:bg-slate-800/30 transition-all">
            <div className="flex items-center gap-3"><Users className="text-emerald-500" size={24} /><h2 className="text-xl font-black uppercase italic tracking-tight">Iscrizioni ({profiles.length})</h2></div>
              ...
              {profiles.map(p => (
                <div key={p.id} className="p-4 flex items-center justify-between hover:bg-slate-800/20">
                  <div className="overflow-hidden">
                    <p className="font-black text-sm uppercase italic text-white truncate">{p.username} <span className="text-yellow-500">#{p.ranking || '--'}</span></p>
                    <p className="text-[9px] text-slate-500 mt-1">{p.points || 0} PT ({p.points_groups}G+{p.points_bracket}B) - Esatti: {p.exact_matches || 0}</p>
                  </div>
                  <div className="flex gap-2 shrink-0">
                    <button onClick={() => togglePayment(p.id, p.is_paid)} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase ${p.is_paid ? 'bg-emerald-500 text-slate-950' : 'bg-slate-950 text-rose-500 border border-rose-500'}`}>{p.is_paid ? 'PAGATO' : 'DA PAGARE'}</button>
                    <button onClick={() => deleteUser(p.id, p.username)} className="p-2 text-rose-500 hover:bg-rose-500/10 rounded-lg"><Trash2 size={16} /></button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* SEZIONE 2: RISULTATI GIRONI (CON BANDIERE) */}
        <section className="bg-slate-900/50 border border-slate-800 rounded-[2rem] overflow-hidden shadow-xl">
          <button onClick={() => setOpenSection({ ...openSection, risultati: !openSection.risultati })} className="w-full p-6 flex items-center justify-between hover:bg-slate-800/30 transition-all">
            <div className="flex items-center gap-3"><Zap className="text-yellow-500" size={24} /><h2 className="text-xl font-black uppercase italic tracking-tight">Risultati Gironi</h2></div>
          ...
          {openSection.risultati && (
            <div className="p-6 space-y-4">
              <div className="relative">
                ...
                <input type="text" placeholder="CERCA SQUADRA..." className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-10 pr-4 py-4 text-xs font-black uppercase outline-none focus:border-yellow-500" onChange={e => setSearchTerm(e.target.value)} />
              </div>
              <div className="grid gap-3">
                {matches.filter(m => m.home_team.toLowerCase().includes(searchTerm.toLowerCase()) || m.away_team.toLowerCase().includes(searchTerm.toLowerCase())).map(m => {
                  ...
                  return (
                    <div key={m.id} className={`bg-slate-900 p-4 rounded-3xl border transition-all flex items-center gap-4 ${hasR ? 'border-emerald-500/30 shadow-lg shadow-emerald-500/5' : 'border-slate-800'}`}>
                      <span className="text-[10px] font-black text-slate-600 w-6">#{m.id}</span>
                      <div className="flex-1 flex items-center justify-center gap-3">
                        {/* Casa */}
                        <div className="flex-1 flex items-center justify-end gap-2 overflow-hidden text-right">
                          <span className="text-[10px] font-black uppercase truncate italic">{m.home_team}</span>
                          <img src={`https://flagcdn.com/w40/${flagMap[m.home_team.toLowerCase()] || 'un'}.png`} className="w-5 h-3.5 object-cover rounded shadow-sm shrink-0 border border-white/5" alt="" />
                        </div>
                        {/* Input Score */}
                        <div className="flex items-center gap-1 shrink-0">
                          <input id={`h-${m.id}`} type="number" defaultValue={m.home_score_final ?? ''} className="w-9 h-9 bg-slate-950 rounded-lg text-center font-black text-yellow-500 border border-slate-700 outline-none focus:border-yellow-500" />
                          <span className="text-slate-700 font-black text-xs">-</span>
                          <input id={`a-${m.id}`} type="number" defaultValue={m.away_score_final ?? ''} className="w-9 h-9 bg-slate-950 rounded-lg text-center font-black text-yellow-500 border border-slate-700 outline-none focus:border-yellow-500" />
                        </div>
                        {/* Trasferta */}
                        <div className="flex-1 flex items-center justify-start gap-2 overflow-hidden text-left">
                          <img src={`https://flagcdn.com/w40/${flagMap[m.away_team.toLowerCase()] || 'un'}.png`} className="w-5 h-3.5 object-cover rounded shadow-sm shrink-0 border border-white/5" alt="" />
                          <span className="text-[10px] font-black uppercase truncate italic">{m.away_team}</span>
                        </div>
                      </div>
                      <button onClick={() => updateScore(m.id)} className={`p-2 rounded-lg font-black uppercase text-[8px] tracking-widest ${hasR ? 'bg-emerald-500 text-slate-950' : 'bg-yellow-500 text-slate-950'}`}>
                        {hasR ? 'MOD' : 'SALVA'}
                      </button>
                    </div>
                  );
        ...
        <section className="bg-slate-900/50 border border-slate-800 rounded-[2rem] overflow-hidden shadow-xl">
          <button onClick={() => setOpenSection({ ...openSection, tabellone: !openSection.tabellone })} className="w-full p-6 flex items-center justify-between hover:bg-slate-800/30 transition-all">
            <div className="flex items-center gap-3"><Trophy className="text-blue-500" size={24} /><h2 className="text-xl font-black uppercase italic tracking-tight">Fase Finale</h2></div>
            {openSection.tabellone ? <ChevronUp /> : <ChevronDown />}
          </button>
          {openSection.tabellone && (
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <select id="q_team" className="bg-slate-950 border border-slate-800 p-4 rounded-xl font-black text-xs text-white uppercase outline-none focus:border-blue-500"><option value="">SQUADRA...</option>{TEAMS_2026.map(t => (<option key={t} value={t}>{t}</option>))}</select>
                <select id="q_stage" className="bg-slate-950 border border-slate-800 p-4 rounded-xl font-black text-xs text-white uppercase outline-none focus:border-blue-500">{STAGES.map(s => (<option key={s.id} value={s.id}>{s.label}</option>))}</select>
              </div>
              <button onClick={saveQualif} className="w-full bg-blue-600 hover:bg-blue-500 py-4 rounded-xl font-black uppercase text-xs tracking-widest active:scale-95 shadow-lg shadow-blue-600/20 transition-all">CONFERMA QUALIFICATA</button>
              <div className="space-y-4">
                {STAGES.map(stg => {
                  const items = officialBracket.filter(o => normalizeStage(o.stage) === stg.id);
                  if (items.length === 0) return null;
                  return (
                    <div key={stg.id} className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800">
                      <h3 className="text-[10px] font-black text-blue-500 uppercase mb-3 border-b border-slate-800 pb-2 tracking-widest">{stg.label}</h3>
                      <div className="flex flex-wrap gap-2">
                        {items.map(o => (
                          <div key={o.id} className="bg-slate-900 border border-slate-700 px-3 py-2 rounded-xl flex items-center gap-3">
                            <span className="text-[10px] font-black uppercase italic text-white">{o.team_name}</span>
                            <button onClick={() => deleteQualif(o.id)} className="text-rose-500 hover:text-rose-400"><Trash2 size={12} /></button>
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        <section className="bg-slate-900/50 border border-slate-800 rounded-[2rem] overflow-hidden shadow-xl">
          <button onClick={() => setOpenSection({ ...openSection, bonus: !openSection.bonus })} className="w-full p-6 flex items-center justify-between hover:bg-slate-800/30 transition-all">
            <div className="flex items-center gap-3"><Star className="text-purple-500" size={24} /><h2 className="text-xl font-black uppercase italic tracking-tight">Bonus Ufficiali</h2></div>
            ...
            <div className="p-6 border-t border-slate-800 bg-slate-900/30">
              <form onSubmit={saveBonuses} className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="space-y-1"><span className="flex justify-between text-[9px] font-black uppercase text-slate-500 ml-1"><span>MVP (10pt)</span></span><input value={bonusData.mvp_world_cup} onChange={e => setBonusData({ ...bonusData, mvp_world_cup: e.target.value })} className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl font-black uppercase text-purple-400 outline-none focus:border-purple-500" /></div>
                  <div className="space-y-1"><span className="flex justify-between text-[9px] font-black uppercase text-slate-500 ml-1"><span>Capocannoniere (10pt)</span></span><input value={bonusData.top} onChange={e => setBonusData({ ...bonusData, top: e.target.value })} className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl font-black uppercase text-purple-400 outline-none focus:border-purple-500" /></div>
                  <div className="space-y-1"><span className="flex justify-between text-[9px] font-black uppercase text-slate-500 ml-1"><span>Portiere (10pt)</span></span><input value={bonusData.best_goalkeeper} onChange={e => setBonusData({ ...bonusData, best_goalkeeper: e.target.value })} className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl font-black uppercase text-purple-400 outline-none focus:border-purple-500" /></div>
                </div>
                <div className="space-y-1"><span className="text-[9px] font-black uppercase text-slate-500 ml-1">Match con più gol (5pt)</span><select value={bonusData.high} onChange={e => setBonusData({ ...bonusData, high: e.target.value })} className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl font-black uppercase text-blue-400 outline-none focus:border-blue-500">{matches.filter(m => !m.home_team.includes('TBD')).map(m => (<option key={m.id} value={`${m.home_team} - ${m.away_team}`}>{m.home_team} - {m.away_team}</option>))}</select></div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1"><span className="text-[9px] font-black uppercase text-slate-500">Girone + Gol</span><select value={bonusData.high_group} onChange={e => setBonusData({ ...bonusData, high_group: e.target.value })} className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl font-black uppercase text-blue-400">{GROUPS.map(g => (<option key={g} value={g}>{g}</option>))}</select></div>
                  <div className="space-y-1"><span className="text-[9px] font-black uppercase text-slate-500">Girone - Gol</span><select value={bonusData.low_group} onChange={e => setBonusData({ ...bonusData, low_group: e.target.value })} className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl font-black uppercase text-blue-400">{GROUPS.map(g => (<option key={g} value={g}>{g}</option>))}</select></div>
                </div>
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-1"><span className="text-[9px] font-black uppercase text-slate-500">Autogol (3pt)</span><input value={bonusData.own_goals} onChange={e => setBonusData({...bonusData, own_goals: e.target.value})} type="number" className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl font-black text-emerald-400 outline-none" /></div>
                  <div className="space-y-1"><span className="text-[9px] font-black uppercase text-slate-500">Rigori (3pt)</span><input value={bonusData.penalties} onChange={e => setBonusData({...bonusData, penalties: e.target.value})} type="number" className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl font-black text-emerald-400 outline-none" /></div>
                  <div className="space-y-1"><span className="text-[9px] font-black uppercase text-slate-500">Rossi (3pt)</span><input value={bonusData.red} onChange={e => setBonusData({...bonusData, red: e.target.value})} type="number" className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl font-black text-emerald-400 outline-none" /></div>
                </div>
                <div className="flex gap-3 pt-4"><button type="button" onClick={resetBonuses} className="p-5 bg-slate-900 border border-rose-500/30 text-rose-500 rounded-2xl hover:bg-rose-500 hover:text-white transition-all"><Trash2 size={20} /></button><button type="submit" className="flex-1 bg-purple-600 hover:bg-purple-500 py-5 rounded-2xl font-black uppercase tracking-widest text-xs italic shadow-xl shadow-purple-600/20 transition-all active:scale-95">SALVA BONUS UFFICIALI</button></div>
              </form>
            </div>
          )}
        </section>

        {/* SEZIONE 5: STATISTICHE (VERSIONE COMPLETA) */}
        <section className="bg-slate-900/50 border border-slate-800 rounded-[2rem] overflow-hidden shadow-xl">
          <button onClick={() => setOpenSection({ ...openSection, statistiche: !openSection.statistiche })} className="w-full p-6 flex items-center justify-between hover:bg-slate-800/30 transition-all">
            <div className="flex items-center gap-3"><BarChart3 className="text-cyan-500" size={24} /><h2 className="text-xl font-black uppercase italic tracking-tight">Statistiche Globali</h2></div>
            {openSection.statistiche ? <ChevronUp /> : <ChevronDown />}
          </button>
          {openSection.statistiche && (
            <div className="p-6 space-y-6 bg-slate-950/30 border-t border-slate-800">
              <div className="grid grid-cols-3 gap-3 text-center">
                <div className="bg-slate-900 p-4 rounded-2xl border border-slate-800"><p className="text-[8px] font-black text-slate-500 uppercase mb-1">Rossi Avg</p><p className="text-2xl font-black text-cyan-400">{getAverage('total_red_cards')}</p></div>
                <div className="bg-slate-900 p-4 rounded-2xl border border-slate-800"><p className="text-[8px] font-black text-slate-500 uppercase mb-1">Rigori Avg</p><p className="text-2xl font-black text-cyan-400">{getAverage('total_penalties')}</p></div>
                <div className="bg-slate-900 p-4 rounded-2xl border border-slate-800"><p className="text-[8px] font-black text-slate-500 uppercase mb-1">Autogol Avg</p><p className="text-2xl font-black text-cyan-400">{getAverage('total_own_goals')}</p></div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
                {[
                  ...
                  <div key={s.key} className="bg-slate-900 p-4 rounded-2xl border border-slate-800 shadow-md">
                    <p className="text-[9px] font-black text-slate-500 uppercase mb-3 tracking-widest border-b border-slate-800 pb-1 italic">{s.label} (Top Voti)</p>
                    ...
                    {getTopPicks(s.key).length === 0 && <p className="text-[8px] text-slate-700 italic text-center py-2 uppercase">Dati non disponibili</p>}
                  </div>
                ))}
              </div>
            </div>
