...
export default function AdminPage() {
  ...
  const syncLeaderboard = async (isManual = true) => {
    if (
      isManual &&
      !window.confirm(
        'Vuoi ricalcolare i punti e la classifica per tutti i giocatori?'
      )
    )
      return;

    setSyncing(true);
    const syncToast = !isManual
      ? toast.loading('Aggiornamento classifica...')
      : null;
    ...
    try {
      ...
      const updates = profs.map((profile) => {
        ...
        // 2. CALCOLO TABELLONE
        const myBrackets =
          userBrackets?.filter((b) => b.user_id === profile.id) || [];
        myBrackets.forEach((ub) => {
          const uStg = normalizeStage(ub.stage);
          if (
            offBracket?.some(
              (ob) =>
                normalizeStage(ob.stage) === uStg &&
                ob.team_name.toLowerCase().trim() ===
                  ub.team_name.toLowerCase().trim()
            )
          ) {
  ...
  const resetBonuses = async () => {
    if (
      !window.confirm(
        'Vuoi davvero svuotare TUTTI i bonus ufficiali? I punti degli utenti verranno ricalcolati.'
      )
    )
      return;
    const payload = {
    ...
    if (error) toast.error(error.message);
      ...
      toast.success('Bonus azzerati con successo!', { icon: '🧹' });
      setBonusData({
        red: '',
        top: '',
        high: '',
  ...
  const saveQualif = async () => {
    ...
    const stage = (document.getElementById('q_stage') as HTMLSelectElement)
      .value;
  ...
  const deleteUser = async (userId: string, username: string) => {
    if (
      !window.confirm(
        `ATTENZIONE! Vuoi davvero eliminare l'utente "${username}"? L'azione è irreversibile e cancellerà anche tutti i suoi pronostici!`
      )
    )
      return;
    setSyncing(true);
    try {
      ...
      toast.success(`Utente ${username} eliminato correttamente.`, {
        icon: '🗑️',
      });
      fetchData();
      await syncLeaderboard(false);
  ...
  if (!isAdmin)
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center text-rose-500 font-black border-4 border-rose-500 p-10">
        ACCESSO NEGATO
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-950 text-white p-4 pb-32 font-sans">
      ...
      <div className="max-w-3xl mx-auto space-y-6 text-left">
        ...
        <section className="bg-slate-900/50 border border-slate-800 rounded-[2rem] overflow-hidden shadow-xl">
          <button
            onClick={() =>
              setOpenSection({
                ...openSection,
                iscrizioni: !openSection.iscrizioni,
              })
            }
        ...
        <section className="bg-slate-900/50 border border-slate-800 rounded-[2rem] overflow-hidden shadow-xl">
          <button
            onClick={() =>
              setOpenSection({
                ...openSection,
                risultati: !openSection.risultati,
              })
            }
            className="w-full p-6 flex items-center justify-between hover:bg-slate-800/30 transition-all"
          >
            ...
            <div className="border-t border-slate-800 p-6 space-y-4">
              ...
              <div className="grid gap-3">
                {matches
                  ...
                  .map((m) => {
                    const hasResult =
                      m.is_finished && m.home_score_final !== null;
                    return (
                      <div
                        key={m.id}
                        className={`bg-slate-900 p-4 rounded-3xl border transition-all flex items-center gap-4 ${
                          hasResult
                      ...
                      >
                        ...
                        <div className="flex-1 flex items-center justify-center gap-2">
                          <span className="text-[10px] font-black uppercase flex-1 text-right truncate italic">
                            {m.home_team}
                          </span>
                          <input
                            id={`h-${m.id}`}
                            ...
                            className="w-10 h-10 bg-slate-950 rounded-lg text-center font-black text-yellow-500 border border-slate-700 outline-none focus:border-yellow-500"
                          />
                          ...
                          <input
                            ...
                            className="w-10 h-10 bg-slate-950 rounded-lg text-center font-black text-yellow-500 border border-slate-700 outline-none focus:border-yellow-500"
                          />
                          <span className="text-[10px] font-black uppercase flex-1 text-left truncate italic">
                            {m.away_team}
                          </span>
                        </div>
                        <button
                          onClick={() => updateScore(m.id)}
                          className={`p-2 rounded-lg font-black uppercase text-[8px] tracking-tighter transition-all active:scale-90 ${
                            hasResult
        ...
        <section className="bg-slate-900/50 border border-slate-800 rounded-[2rem] overflow-hidden shadow-xl">
          <button
            onClick={() =>
              setOpenSection({
                ...openSection,
                tabellone: !openSection.tabellone,
              })
            }
            className="w-full p-6 flex items-center justify-between hover:bg-slate-800/30 transition-all"
          >
          ...
          {openSection.tabellone && (
            <div className="border-t border-slate-800 p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <select
                  id="q_team"
                  className="bg-slate-950 border-2 border-slate-800 p-4 rounded-xl font-black text-xs uppercase outline-none focus:border-blue-500 text-white"
                >
              ...
              <button
                onClick={saveQualif}
                className="w-full bg-blue-600 py-4 rounded-xl font-black uppercase text-xs tracking-widest active:scale-95 shadow-lg shadow-blue-600/20 transition-all"
              >
              ...
              <div className="space-y-4 mt-6">
                {STAGES.map((stg) => {
                  const items = officialBracket.filter(
                    (o) => normalizeStage(o.stage) === stg.id
                  );
                  if (items.length === 0) return null;
                  return (
                    <div
                      key={stg.id}
                      className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800 shadow-inner"
                    >
                      <h3 className="text-[10px] font-black text-blue-500 uppercase mb-3 tracking-[0.2em] border-b border-slate-800 pb-2">
                        {stg.label}
                      </h3>
                      <div className="flex flex-wrap gap-2">
                        {items.map((o) => (
                          <div
                            key={o.id}
                            className="bg-slate-900 border border-slate-700 pl-3 pr-2 py-2 rounded-xl flex items-center gap-3 shadow-md"
                          >
                            <span className="text-[10px] font-black uppercase text-white italic">
                              {o.team_name}
                            </span>
                            <button
        ...
        <section className="bg-slate-900/50 border border-slate-800 rounded-[2rem] overflow-hidden shadow-xl">
          <button
            onClick={() =>
              setOpenSection({ ...openSection, bonus: !openSection.bonus })
            }
            className="w-full p-6 flex items-center justify-between hover:bg-slate-800/30 transition-all"
          >
            <div className="flex items-center gap-3">
              ...
              <form onSubmit={saveBonuses} className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      ...
                      />
                      <input
                        value={bonusData.best_goalkeeper}
                        onChange={(e) =>
                          setBonusData({
                            ...bonusData,
                            best_goalkeeper: e.target.value,
                          })
                        }
                        className="w-full bg-slate-950 border border-slate-800 pl-11 pr-4 py-4 rounded-xl font-black uppercase outline-none focus:border-purple-500 text-purple-400"
                      />
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    ...
                    className="p-5 bg-slate-900 border border-rose-500/30 text-rose-500 rounded-2xl hover:bg-rose-500 hover:text-white transition-all group"
                    title="Svuota tutti i bonus"
                  >
                    <Trash2
                      size={20}
        ...
        <section className="bg-slate-900/50 border border-slate-800 rounded-[2rem] overflow-hidden shadow-xl transition-all">
          <button
            onClick={() =>
              setOpenSection({
                ...openSection,
                statistiche: !openSection.statistiche,
              })
            }
            className="w-full p-6 flex items-center justify-between hover:bg-slate-800/30 transition-all"
          >
            ...
            <div className="border-t border-slate-800 p-6 space-y-6 bg-slate-950/30">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  >
                    <p className="text-[9px] font-black text-slate-500 uppercase mb-3 tracking-widest border-b border-slate-800 pb-1">
                      {s.label} (Top Voti)
                    </p>
                    {getTopPicks(s.key).map(([name, count]: any) => (
                      <div
                        key={name}
                        className="flex justify-between text-[10px] font-black uppercase italic mb-1"
                      >
                        <span className="truncate pr-2 text-white">{name}</span>
                        <span className="text-cyan-500 shrink-0 font-mono">
                          {count} voti
                        </span>
                      </div>
                    ))}
                    {getTopPicks(s.key).length === 0 && (
                      <p className="text-[8px] text-slate-700 italic">
                        Nessun pronostico inserito dagli utenti
                      </p>
                    )}
                  </div>
                ))}
